...
/* why do it this way?  We need to read lots of sectors to kludge
  ...
  // AF FIXME LATER, allocate extra space for C2 error codes
  if(p->enable&(PARANOIA_MODE_OVERLAP|PARANOIA_MODE_VERIFY)){
    flags=paranoia_calloc(totaltoread* ( CD_FRAMEWORDS + 300 ),1);
    new=new_c_block(p);
    recover_cache(p);
  }else{
  ...
  // AF FIXME LATER, allocate extra space for C2 error codes
  buffer=paranoia_malloc(totaltoread* ( CD_FRAMESIZE_RAW + 300 ) );
  sofar=0;
  firstread=-1;
