...
struct CDROMPARAMS
{
	...
	char 		lpszVendorID[9];        // CD-ROM vendor ID
	char 		lpszProductID[17];      // CD-ROM Product ID
	...
	BOOL		bFUA;					// FUA enabled
	LONG		nSampleOffset;          // Sample offset, in samples, so byte offset = 2 * sampleoffset * 2 bytes
    BOOL		bReadC2Errors;
};
...
DLLEXPORT CDEX_ERR CCONV CR_Init( int nTransportLayer, int debugLevel );
...
DLLEXPORT CDEX_ERR CCONV CR_OpenRipper(	LONG* plBufferSize,LONG dwStartSector,LONG dwEndSector, BOOL bExcludeNullSamplesInCRC );
...
DLLEXPORT CDEX_ERR CCONV CR_ReadSubchannelData( 
    BYTE* pbtStream,
    DWORD numBytes, 
    LONG sector, 
    DWORD numSectors   );
