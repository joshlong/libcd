...
static void	copy_filename (SF_PRIVATE *psf, const tchar *path) ;
...
SNDFILE*
sf_open	(const tchar *path, int mode, SF_INFO *sfinfo)
{	SF_PRIVATE 	*psf ;
	int			error ;
...
{	SF_PRIVATE 		*psf = NULL ;
	...
	{	case SFC_SET_NORM_FLOAT :
		...
		case SFC_CALC_MAX_ALL_CHANNELS :
			if (! data || datasize != SIGNED_SIZEOF (double) * psf->sf.channels)
				return (psf->error = SFE_BAD_CONTROL_CMD) ;
		...
		case SFC_SET_ADD_DITHER_ON_WRITE :
			if (psf->mode == SFM_WRITE || psf->mode == SFM_RDWR)
				psf->write_dither = datasize ? SF_TRUE : SF_FALSE ;
			break ;

		case SFC_SET_ADD_DITHER_ON_READ :
			if (psf->mode == SFM_READ || psf->mode == SFM_RDWR)
				psf->read_dither = datasize ? SF_TRUE : SF_FALSE ;
			break ;
		...
		case SFC_FILE_TRUNCATE :
			...
			{   sf_count_t position ;
				...
				position = psf_fseek (psf->filedes, 0, SEEK_CUR) ;

				return psf_ftruncate (psf->filedes, position) ;
				} ;
			break ;
...
static void
copy_filename (SF_PRIVATE *psf, const tchar *path)
{	
	const tchar *cptr = NULL;
	...
	if ((cptr = tcsrchr (path, _T( '/' ) ) ) || (cptr = tcsrchr (path, _T( '\\' ) ) ) )
		cptr ++ ;
	else
		cptr = path ;

	i = ( cptr - path ) / sizeof( tchar );

	while ( ( path[ i ] != _T( '\0' ) ) && ( i < SF_FILENAME_LEN ) )
	{
		psf->filename[ i ] = (char)( path[ i ] & 0xFF );
		i++;
	}
