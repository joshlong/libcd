/* $Id: interface.c 271 2001-10-14 20:36:50Z afaber $ */
...
static struct buf *addbuf( PMPSTR mp, unsigned char *buffer_data,int size)
{
	struct buf *nbuf = NULL;
	int nSize = sizeof( struct buf );

	nbuf = (struct buf*) malloc( nSize );
	if(!nbuf) {
	...
	nbuf->size = size;
	memcpy(nbuf->pnt, buffer_data,(size_t)size);
	nbuf->next = NULL;
	nbuf->prev = mp->head;
	nbuf->pos = 0;
