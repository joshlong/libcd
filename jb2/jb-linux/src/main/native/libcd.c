...
static void WriteWav(int f, long bytes) {
	...
	ssize_t sst;
	sst = write(f, "RIFF", 4); /*  0-3 */
	PutNum(bytes + 44 - 8, f, 0, 4); /*  4-7 */
	sst = write(f, "WAVEfmt ", 8); /*  8-15 */
...
PRBool _cdripper_rip_track(char *device, char *generic_scsi_device, int track,
	...
	while (cursor <= last_sector) {
		/* read a sector */
		PRInt16 *readbuf = paranoia_read(p,
				&_cdripper_rip_track_progress_update);
...
PRBool cdripper_rip_track(char * device_name, int track, char * outfile,
	...
	PRBool did_it_work = _cdripper_rip_track(device_name, NULL, 1, start_lsn,
			stop_lsn, outfile, PARANOIA_MODE_DISABLE, rip_percent_done,
			&stop_thread_rip_now, PR_FALSE);
