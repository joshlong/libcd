// $Id: misc_support.cpp 596 2007-01-01 19:17:39Z afaber $
...
char *ID3_GetSongLen(const ID3_Tag *tag)
{
  char *sSongLen = NULL;
  if (NULL == tag)
  {
  ...
  ID3_Frame *pFrame = tag->Find(ID3FID_SONGLEN);
  if (pFrame != NULL)
  {
    sSongLen = ID3_GetString(pFrame, ID3FN_TEXT);
  }
  return sSongLen;
}
...
size_t ID3_RemoveSongLen(ID3_Tag *tag)
{
  size_t nRemoved = 0;
  ID3_Frame *pFrame = NULL;
  ...
  while ((pFrame = tag->Find(ID3FID_SONGLEN)))
  {
...
size_t ID3_RemoveMCDI(ID3_Tag *tag)
{
  size_t nRemoved = 0;
  ID3_Frame *pFrame = NULL;
  ...
  while ((pFrame = tag->Find(ID3FID_CDID)))
  {
    tag->RemoveFrame(pFrame);
    nRemoved++;
  }
...
char *ID3_GetEncodedBy(const ID3_Tag *tag)
{
  ...
  ID3_Frame *pFrame = tag->Find(ID3FID_ENCODEDBY);
  if (pFrame != NULL)
  {
    sComment = ID3_GetString(pFrame, ID3FN_TEXT);
  }
...
size_t ID3_RemoveEncodedBy(ID3_Tag *tag)
{
  ...
  while ((pFrame = tag->Find(ID3FID_ENCODEDBY)))
  {
    tag->RemoveFrame(pFrame);
    nRemoved++;
  }
